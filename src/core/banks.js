...
import { SOUND, SOUNDS, registerSound, unregisterSound, placeholderSound } from './sounds.js';

export const hiddenBanks = new Set();   // banks kept loaded (songs may use them) but left out of pickers and the Sounds table
export const isHidden = soundId => { const b = SOUND[soundId] && SOUND[soundId].bank; return !!b && hiddenBanks.has(b); };
...
banks.set('orchestra', { id: 'orchestra', name: 'Symphony orchestra', description: 'Woodwinds, brass, timpani, harp, strings and a choir from VSCO 2 CE and VCSL, plus two synths. Loaded by default.', license: 'CC0-1.0', url: '', instruments: SOUNDS.map(i => i.id), includes: [] });
...
export function installBank(json, url) {
  ...
  for (const def of json.instruments) {
    ...
    registerSound(d, json.id); ids.push(d.id);
  }
  const includes = (json.include || []).filter(id => SOUND[id]);
...
export function unloadBank(id, inUse = () => false) {
  ...
  for (const iid of b.instruments) if (!inUse(iid)) unregisterSound(iid);
...
export function placeholdersFor(song) {
  const made = [];
  for (const tr of song.instruments) if (!SOUND[tr.sound]) { placeholderSound(tr.sound); made.push(tr.sound); }
  return made;
}
...
export async function ensureSongBanks(song) {
  ...
  for (const tr of song.instruments) if (!SOUND[tr.sound] || SOUND[tr.sound].bank === 'missing') { if (!SOUND[tr.sound]) placeholderSound(tr.sound); missing.push(tr.sound); }
  return missing;
}
export const bankOf = soundId => (SOUND[soundId] || {}).bank || null;
