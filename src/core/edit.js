// Editing primitives on pattern material: note lookup and indexing, the note-overlap rules, length limits,
...
export function indexTrack(pat, trackId) { return indexMaterial(pat, pat.material[trackId]); }
// Same, for any material shape (a track's loose material, or the notes a placement produces).
export function indexMaterial(pat, pt) {
...
export function noteAt(pat, trackId, col, row) {
  const pt = pat.material[trackId]; if (!pt) return null;
  return pt.notes.find(e => e.col === col && rowOf(pat, e.tick) === row) || null;
}
...
export function notesStartingAt(pat, trackId, row) {
  const pt = pat.material[trackId]; if (!pt) return [];
  return pt.notes.filter(e => rowOf(pat, e.tick) === row);
}
export function notesIn(pat, trackId, col, r0, r1) {
  const pt = pat.material[trackId]; if (!pt) return [];
  return pt.notes.filter(e => e.col === col && rowOf(pat, e.tick) >= r0 && rowOf(pat, e.tick) <= r1);
}
...
export function setNote(pat, trackId, col, tick, pitch, len) {
  const pt = materialOf(pat, trackId);
...
export function removeNotesAt(pat, trackId, col, row) {
  const pt = pat.material[trackId]; if (!pt) return;
  pt.notes = pt.notes.filter(e => !(e.col === col && rowOf(pat, e.tick) === row));
}
...
export function fxAtRow(pat, trackId, row) {
  const pt = pat.material[trackId]; if (!pt || !pt.fx) return null;
...
export function removeFx(pat, trackId, tick) {
  const pt = pat.material[trackId]; if (!pt || !pt.fx) return;
