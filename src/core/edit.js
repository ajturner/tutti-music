...
export function indexInstrument(phr, instrumentId) { return indexMaterial(phr, phr.material[instrumentId]); }
// Same, for any material shape (a instrument's loose material, or the notes a placement produces).
...
export function noteAt(phr, instrumentId, col, row) {
  const pt = phr.material[instrumentId]; if (!pt) return null;
  return pt.notes.find(e => e.col === col && rowOf(phr, e.tick) === row) || null;
}
export function noteCovering(phr, instrumentId, col, row) {
  const pt = phr.material[instrumentId]; if (!pt) return null;
  const tpr = phr.ticksPerRow;
...
export function notesStartingAt(phr, instrumentId, row) {
  const pt = phr.material[instrumentId]; if (!pt) return [];
  return pt.notes.filter(e => rowOf(phr, e.tick) === row);
}
export function notesIn(phr, instrumentId, col, r0, r1) {
  const pt = phr.material[instrumentId]; if (!pt) return [];
  return pt.notes.filter(e => e.col === col && rowOf(phr, e.tick) >= r0 && rowOf(phr, e.tick) <= r1);
}
export function nextNote(phr, instrumentId, col, tick) {
  const pt = phr.material[instrumentId]; if (!pt) return null;
...
export function maxLength(phr, instrumentId, col, tick) {
  const next = nextNote(phr, instrumentId, col, tick);
...
export function setNote(phr, instrumentId, col, tick, pitch, len) {
  const pt = materialOf(phr, instrumentId);
  ...
  const ev = { tick, len: clamp(len, phr.ticksPerRow, maxLength(phr, instrumentId, col, tick)), pitch, vel: 100, col, art: null };
  pt.notes.push(ev);
  return ev;
}
// Replace whatever starts at tick with a complete note (used by paste).
export function putNote(phr, instrumentId, col, tick, n) {
  const pt = materialOf(phr, instrumentId);
  pt.notes = pt.notes.filter(e => !(e.col === col && e.tick === tick));
  for (const e of pt.notes) if (e.col === col && e.tick < tick && e.tick + e.len > tick) e.len = tick - e.tick;
  const ev = { tick, len: clamp(n.len, phr.ticksPerRow, maxLength(phr, instrumentId, col, tick)), pitch: n.pitch, vel: n.vel, col, art: n.art || null };
  pt.notes.push(ev);
  return ev;
}
export function removeNotesAt(phr, instrumentId, col, row) {
  const pt = phr.material[instrumentId]; if (!pt) return;
  pt.notes = pt.notes.filter(e => !(e.col === col && rowOf(phr, e.tick) === row));
}
// Change a note's length by rows within the column limits.
export function resizeNote(phr, instrumentId, ev, dRows) {
  ev.len = clamp(ev.len + dRows * phr.ticksPerRow, phr.ticksPerRow, maxLength(phr, instrumentId, ev.col, ev.tick));
  return ev;
}

// ---- FX column ------------------------------------------------------------------------------
export function fxAtRow(phr, instrumentId, row) {
  const pt = phr.material[instrumentId]; if (!pt || !pt.fx) return null;
  return pt.fx.find(f => f.tick === row * phr.ticksPerRow) || null;
}
export function setFx(phr, instrumentId, tick, cmd, value) {
  const pt = materialOf(phr, instrumentId);
...
export function removeFx(phr, instrumentId, tick) {
  const pt = phr.material[instrumentId]; if (!pt || !pt.fx) return;
