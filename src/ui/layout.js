// Grid geometry: cell positions per instrument and the cell under the cursor.
import { clamp } from '../core/constants.js';
import { PAD, state, instrumentsShown, view } from './state.js';

// ---- Layout -----------------------------------------------------------------------
// The visible cells of a instrument, in order: per note column a note and (if shown) a velocity cell, then
...
export const HEADER_ROWS = 3;   // family band, instrument name, cell labels
export function computeLayout() {
  const cw = view.charW;
  const gutter = { rowX: PAD, tempoX: PAD + cw * 4.5, w: PAD + cw * 10 };
  let x = gutter.w, g = 1;
  const instruments = instrumentsShown().map(tr => {
    ...
    cx += 0.5 * cw;
    const lay = { x, w: cx - x, cells, instrument: tr, g0: g };
    x = cx; g += cells.length;
    return lay;
  });
  return { gutter, instruments, totalW: x };
}
export function currentCell(L) {
  if (state.cursor.instrument < 0) return { kind: 'tempo', col: 0 };
  const lay = (L || computeLayout()).instruments[state.cursor.instrument];
  if (!lay) { state.cursor.instrument = (L || computeLayout()).instruments.length - 1; return currentCell(L); }   // the instrument under the cursor is gone: the last one, or the tempo column of a song with none
