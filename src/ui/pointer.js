...
export function placeCursor(hit, extend) {
  ...
  state.cursor.row = hit.row; state.cursor.track = hit.track; state.cursor.cell = hit.cell;
...
canvas.addEventListener('pointermove', e => {
  ...
  if (!drag.moved) {
    ...
    if (drag.select && drag.hit) {   // anchor the selection where the drag began (or at the cursor with shift)
      ...
      if (!state.selAnchor) state.selAnchor = { row: drag.hit.row, g: cellIndex(drag.hit.track, drag.hit.cell) };
    }
  }
  if (drag.select) {
    if (!drag.hit) return;
