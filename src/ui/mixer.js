// Mixer sidebar: one strip per instrument beside the grid, live while editing. Name jumps the cursor to the
// instrument; M and S toggle mute and solo; sliders set volume (CC7) and pan (CC10) and send them while playing.
import { SOUND } from '../core/sounds.js';
import { $, state } from './state.js';
import { noteName } from '../core/constants.js';
import { soundingNow } from './monitor.js';
import { sendControl, openInstruments } from './instruments.js';
...
export function syncMixer() {
  ...
  syncLive();
  const instruments = state.song.instruments, anySolo = instruments.some(t => t.solo);
  const s = instruments.map(t => [t.id, t.name, t.mute ? 1 : 0, t.solo ? 1 : 0, vol(t), pan(t)].join('|')).join(';') + '#' + state.cursor.instrument;
  if (s === sig) return; sig = s;
  const box = $('mixerStrips');
  // Rebuild structure only when the instrument list changed; otherwise update values in place so sliders keep focus.
  const same = box.children.length === instruments.length && [...box.children].every((el, i) => el.dataset.id === instruments[i].id);
  if (!same) {
    box.innerHTML = instruments.map((t, i) => `
      <div class="strip" data-id="${t.id}" data-i="${i}">
        <button class="name" data-act="go" title="Go to this instrument"></button>
        <span class="live" title="What this instrument is sounding now"></span>
  ...
  [...box.children].forEach((el, i) => {
    const t = instruments[i], silent = t.mute || (anySolo && !t.solo);
    el.classList.toggle('cur', i === state.cursor.instrument);
    const name = el.querySelector('.name'); name.textContent = t.name; name.classList.toggle('silent', silent);
    name.style.color = silent ? '' : (SOUND[t.sound] ? '' : '');
...
// The instrument monitor: the note each strip is sounding and how hard, like the M8's instrument readout.
function syncLive() {
...
export function wireMixer() {
  ...
  box.addEventListener('click', e => {
    ...
    const i = parseInt(b.closest('.strip').dataset.i, 10), t = state.song.instruments[i];
    if (b.dataset.act === 'go') { leavePattern(); state.songCursor.instrument = i; state.cursor.instrument = i; state.cursor.cell = 0; state.ensureVisible = true; $('grid').focus(); }
  ...
  box.addEventListener('input', e => {
    ...
    const i = parseInt(e.target.closest('.strip').dataset.i, 10), t = state.song.instruments[i], v = parseInt(e.target.value, 10);
  ...
  $('mixerInstruments').onclick = openInstruments;
}
