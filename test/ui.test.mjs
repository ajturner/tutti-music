...
// ---------- desktop
{
  ...
  // --- FX column ---
  await page.evaluate(() => { state.cursor.track = 0; state.cursor.cell = state.song.instruments[0].columns * 2 + 2; state.cursor.row = 0; state.dirty = true; });
  ...
  check('solo: shift-click header solos the track', await page.evaluate(() => state.song.instruments[0].solo === true));
  await page.keyboard.down('Shift'); await page.mouse.click(box.x + 130, box.y + 30); await page.keyboard.up('Shift'); await page.waitForTimeout(30);
  ...
  const nTracks = await page.evaluate(() => state.song.instruments.length);
  await page.selectOption('#instAddSound', 'synth-arp'); await page.click('#instAdd'); await page.waitForTimeout(50);
  const added = await page.evaluate(() => ({ n: state.song.instruments.length, last: state.song.instruments[state.song.instruments.length - 1], cursor: state.cursor.track, cells: allCells().length }));
  check('instruments: add appends an instrument made from the chosen sound and moves the cursor there', added.n === nTracks + 1 && added.last.sound === 'synth-arp' && added.last.cents === 0 && added.cursor === nTracks, JSON.stringify(added.last));
  ...
  const twin = await page.evaluate(() => { const l = state.song.instruments, a = l[l.length - 2], b = l[l.length - 1]; return { n: l.length, a: a.name, b: b.name, sound: b.sound, vol: b.volume, ids: a.id !== b.id, ch: a.channel !== b.channel, open: document.querySelectorAll('#instList .inst:last-child .instMore').length, focus: document.activeElement.dataset.f, cursor: state.cursor.track }; });
  check('instruments: duplicate adds another player from the same sound with the same settings, open for editing, name selected', twin.n === nTracks + 2 && twin.a === 'Lead' && twin.b === 'Lead 2' && twin.sound === 'synth-arp' && twin.vol === 64 && twin.ids && twin.ch && twin.open === 1 && twin.focus === 'name' && twin.cursor === nTracks + 1, JSON.stringify(twin));
  ...
  check('song view: the Song crumb shows one section with one phrase row and a cell per track', sv0.level === 'song' && sv0.shown && sv0.gridHidden && sv0.secs === 1 && sv0.rows === 1 && sv0.cells === (await page.evaluate(() => state.song.instruments.length)) && sv0.thumbs > 10 && sv0.crumb === 'song' && sv0.focus === 'songView', JSON.stringify(sv0));
  ...
  await page.waitForTimeout(60);
  check('mixer: one strip per track', (await page.$$eval('#mixerStrips .strip', s => s.length)) === (await page.evaluate(() => state.song.instruments.length)));
  ...
  check('song undo: track removed', (await page.evaluate(() => state.song.instruments.length)) === nT - 1);
  await page.evaluate(() => document.getElementById('grid').focus()); await page.keyboard.press('Meta+z'); await page.waitForTimeout(30);
  check('song undo: cmd+Z restores the track', (await page.evaluate(() => state.song.instruments.length)) === nT && (await page.evaluate(() => state.song.instruments[1].id)) === 'ob');
  ...
  // --- real-time record ---
  await page.evaluate(() => { state.preview = false; state.cursor.track = 0; state.cursor.cell = 0; const phr = curPhrase(); phr.material.fl.notes = []; state.song.instruments[0].columns = 1; });
  ...
  check('sounds: one row per sound with source and coverage, and no settings of their own', sounds.rows === (await page.evaluate(() => tutti.INSTRUMENTS.length)) && sounds.v1.startsWith('SMP') && sounds.arp.startsWith('SYN') && sounds.real.join(' ') === 'sus stc piz trm' && sounds.fb === 'leg→sus' && sounds.tuning === 0 && /SMP|SYN|LOAD/.test(sounds.row), JSON.stringify(sounds));
  ...
  await page.evaluate(() => { for (const k of Object.keys(tutti)) if (!(k in window)) Object.defineProperty(window, k, { get: () => tutti[k], configurable: true }); for (const k of ['lastDraw', 'ROW_H']) Object.defineProperty(window, k, { get: () => tutti.view[k], configurable: true }); });
  const bankBack = await page.evaluate(() => ({ loaded: tutti.banks.has('jazz'), inst: !!INST['drum-kit'] && INST['drum-kit'].bank === 'jazz', track: state.song.instruments.some(t => t.sound === 'drum-kit') }));
  ...
  const cols1 = await page.evaluate(() => ({ kinds: tutti.cellKinds(state.song.instruments[0]).map(k => k.kind).join(' '), all: tutti.allCells().filter(c => c.track === 0).map(c => c.kind).join(' '), stored: JSON.parse(localStorage.getItem('tutti.show.v1')) }));
