...
// ---------- phone
{
  ...
  // --- Pocket view: one screen, five levels, the controller's eight buttons on screen ---
  {
    ...
    check('pocket: Back + L goes out a level, to the section', (await pkState()).level === 'section');
    await hold('back', () => tap('lb'));
    check('pocket: and again to the song', (await pkState()).level === 'song');
    ...
    const i0 = await page.evaluate(() => state.pocket.inst.i); await tap('rb');
    check('pocket instruments: R steps to the next instrument', await page.evaluate(i => state.pocket.inst.i === i + 1 && state.cursor.instrument === i + 1, i0));
