...
## Purpose
Lets a composer write a figure once and play it in many places: a pattern is one track's reusable material, a placement plays it at a row with a transpose and a repeat, and editing the pattern changes every placement. Together with a track following a phrase of placements this is how a song grows from phrases into a composition (docs/domain.md).

## Requirements

### Requirement: Pattern
A pattern SHALL be reusable material for one track: a name, an id, a row count, a row size in ticks, a column count and material (notes, dynamics and expression lanes, fx). Patterns SHALL live on the song, SHALL never appear in the arrangement, and SHALL only sound through placements. A pattern belongs to a track kind, not a track: any track may place it.

#### Scenario: Pattern on another track
...
Selecting rows on one track and choosing Make pattern SHALL move that track's loose notes, lane points and fx in those rows into a new pattern named after the track, placed at the first selected row with the columns the notes used. Detach SHALL replace a placement with the loose material it produced. Removing a pattern SHALL detach every placement of it first. Make pattern SHALL refuse rows that already hold a placement and selections spanning more than one track.

#### Scenario: Make pattern
- **WHEN** rows 4 to 35 of the Fiddle track are selected and Make pattern is pressed
...
Copy, cut, duplicate and clear over a track's first note cell SHALL carry the placements starting in the selected rows; paste SHALL place them at the target row on the target track when the pattern exists. Transposing a selection SHALL transpose the loose notes and the placements in it.
