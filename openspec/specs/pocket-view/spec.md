# pocket-view Specification

## Purpose
An optional phone screen in the image of the M8: one fixed screen with a context line, the level you are on (Song, Section, Phrase, Pattern or Instrument), a readout of what every instrument is sounding, and an on-screen eight-button pad that drives the game controller's scheme. It composes a song in Tutti's own model with nothing the next step does not need, one value under the cursor at a time, and is a second renderer over the same song and cursor, so the full interface is one tap away and loses nothing.
