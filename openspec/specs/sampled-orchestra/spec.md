...
The output group SHALL offer samples or synth, remembered per browser, defaulting to samples. Selecting a song, changing the sound, starting playback or adding a track SHALL preload the instruments the song uses, and the status line SHALL show loading progress until decoding completes.
