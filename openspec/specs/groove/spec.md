...
## Purpose
Swing and lilt without editing notes: a per-pattern cycle of row-length multipliers.

## Requirements

### Requirement: Groove definition
A pattern MAY carry a groove: 1 to 16 positive multipliers applied to consecutive rows cyclically. Multipliers SHALL be normalised so the pattern keeps its length. Absent, empty, or all-ones means straight.

#### Scenario: Swung pair
- **WHEN** a pattern with 240-tick rows has groove [1.5, 0.5]
