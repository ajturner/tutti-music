...
### Requirement: Editing
Holding A and pressing up or down SHALL nudge the value under the cursor by one (semitone, velocity, dynamics, bpm, or next articulation). Holding A with left or right SHALL nudge by a large step (12 semitones, 16 for velocity or dynamics, 10 bpm). Tapping A alone SHALL enter the last used pitch on an empty note cell and advance by step, create a default value on an empty dynamics or tempo cell, or audition an existing note. B tapped SHALL clear the cell, or the selection when one exists. X SHALL audition the note under the cursor. Y SHALL cycle articulation.

#### Scenario: Nudge a note
- **WHEN** the cursor is on a D-4 and the user holds A and presses up
